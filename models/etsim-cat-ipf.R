############################################
#### Searching for fast implementation - tring cat::ipf (unsuccessful)

library(cat)
example(ipf)
HairEyeColor # 3d table - I like it... - but all interaction (i.e. processed ind)
m <- c(1,3,0,3,2,0,2,0) # constraints... there are 8 - not sure why
ipf(HairEyeColor, m, showits = T)
ipf()

# Now try on our data - failed 
...
library(car)
ind_b <- ind[2:3]
ind_b$age <- as.numeric(as.character(ind_b$age))
ind_b$age <- recode(ind_b$age, recodes = "1:49 = '16-49'; 50:100 = '50+'")
ind_b

ind_table <- table(ind_b)
all_table <- 
  
loglin(ind_table, margin = list(c(1, 2), c(1, 2)) )
