...
con1 <- read.table("~/IPF-performance-testing/input-data/small-area-eg/hrs_worked.csv", header=TRUE, sep=",")
names(con1) # Check names
con1 <- data.frame(m1 = con1$X1.5, m6 = con1$X6.15, m16 = con1$X16.30, 
                   m31 = con1$X31.37, m38 = con1$X38.48, m49 = con1$X49.or.more,
                   # now females
                   f1 = con1$X1.5.1, f6 = con1$X6.15.1, f16 = con1$X16.30.1, 
...
con2 <- read.table("~/IPF-performance-testing/input-data/small-area-eg/marital_status.csv", header=TRUE, sep=",")
names(con2)
...
### Tenure constraint
con3 <- read.table("~/IPF-performance-testing/input-data/small-area-eg/tenancy.csv", header=TRUE, sep=",")
names(con3)
