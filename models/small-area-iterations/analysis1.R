...
names(mindf4) <- c("observation", "Var2", "Var3", "it", "simulated", "category",
                   "census",  "constraint")
mindf4$Var3 <- mindf4$Var3 - 1

mindf4$constraint <- as.factor(mindf4$constraint)

ggplot(mindf4[-which(mindf4$Var3 ==0),], aes(x=census, y=simulated, 
                                             colour = constraint)) + 
  geom_point(alpha = 0.5) + facet_grid(it ~ Var3)
ggsave("its-3by2.pdf", height = 15, width = 20, unit="cm")