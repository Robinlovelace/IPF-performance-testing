...
source("input-data/small-area-eg/age_sex_gen.R")

conobs <- ls()[grep(pattern = "con[1-9]", x = ls())]
num.cons <- length(conobs)  # calculate n. constraints (can set manually)

sum(con1); sum(con2); sum(con3); sum(con4) # checking that totals add up
# all.msim is the all the constraint variables combined in one data frame
all.msim <- cbind(con1, con2, con3, con4)  # add more constraints if needed
...
con1[con1 == 0] <- con2[con2 == 0] <- con3[con3 == 0] <- con4[con4 == 0] <- 1e-04 # zero values to 0.0001
...
source("models/small-area/categorise-extra-con.R")  # this script must be customised to input data
...
for (i in 1:nrow(all.msim)) {
    ind.agg[i, , 4] <- colSums(ind.cat * weights[, i, num.cons + 1] * 
...
for (it in 2:num.its) {
    source(file = "models/small-area/e2-extra-cons.R")
    wf[, , , it, 1] <- weights
    indf[, , , it, 1] <- ind.agg
}
