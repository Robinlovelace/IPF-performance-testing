...
%DIF DEL /nfs/foe-fs-01_users/georl/Dropbox/txt-backups/ipfinr-b4-eveline-June5th2014.tex   Thu Jun  5 00:12:34 2014
%DIF ADD ipfinr.tex                                                                         Tue Jun 10 18:08:11 2014
...
These include:
\begin{itemize}
 \item Projecting the wider \DIFdelbegin \DIFdel{impacts }\DIFdelend individual and local impacts
of \DIFdelbegin \DIFdel{major job losses from individual companies \mbox{%DIFAUXCMD
\citep{Ballas2006}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{job losses and gains from the closure \mbox{%DIFAUXCMD
\citep{Ballas2006}
}%DIFAUXCMD
or opening \mbox{%DIFAUXCMD
\citep{VanLeeuwen2010}
}%DIFAUXCMD
of new businesses}\DIFaddend .
...
this topic
is provided in \DIFaddbegin \DIFadd{a tutorial entitled
 }\DIFaddend ``Introducing spatial microsimulation with R: a practical'' \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citet{Lovelace2014-ismwr}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{lovelace2014introducing}
}%DIFAUXCMD
}\DIFaddend . This section builds on the tutorial by providing
...
%DIFDELCMD < %%%
\DIFdelend %DIF >  Add correlation matrix at end of links between these
%DIF >  Really should write a paper on this to update Voas2001 - not the place here
...
the input constraint variables. \DIFaddbegin \DIFadd{It is important to think carefully about
...
\DIFdelbegin \DIFdel{This section }\DIFdelend \DIFaddbegin \DIFadd{For dynamic microsimulation scenarios,
...
scenorios are statistically significant \mbox{%DIFAUXCMD
\citep{goedeme2013testing}
}%DIFAUXCMD
.
In static spatial
...
the validity of the process overall \mbox{%DIFAUXCMD
\citep{Lovelace2013-trs}
}%DIFAUXCMD
.
}

\DIFadd{This section briefly }\DIFaddend outlines the evaluation techniques
\DIFdelbegin \DIFdel{that are commonly used in the
field, with a strong focus on internal validation.
}\DIFdelend \DIFaddbegin \DIFadd{used in this study for }\emph{\DIFadd{internal validation}}
\DIFadd{of static spatial microsimulation models.
%DIF >  , in addition to a new implementation of
%DIF >  the S\o rensen-Dice coefficient for continuous variables. % Just making more work!!!
}\DIFaddend The options for external
validation are heavily context dependent so should be decided on a
case-by-case basis.
External validation is thus mentioned in \DIFdelbegin \DIFdel{passing }\DIFdelend \DIFaddbegin \DIFadd{the discussion,
}\DIFaddend with a focus on general principles
rather than specific advice. The \DIFdelbegin \DIFdel{methods }\DIFdelend \DIFaddbegin \DIFadd{three quantitative
methods used in this study (and the visual method of scatter plots)
}\DIFaddend are presented in ascending order of complexity
and roughly descending order of frequency of use, ranging from 
the coefficient of determination ($R^2$) \DIFdelbegin \DIFdel{to entropy based measures.
}\DIFdelend \DIFaddbegin \DIFadd{through total and standardised
absolute error ($TAE$ and $SAE$, respectively)
%DIF >  to the proportion of values deviating from the expected
%DIF >  value by a predefined amount ($P > 0.5$). The preferred metric
to root mean squared error ($RMSE$).
This latter option is our preferred metric, as described below.
%DIF >  The equations for each metric can be found in the associated references.
}

\DIFaddend Before these methods are described, it is
worth stating that the purpose \DIFdelbegin \DIFdel{of this section }\DIFdelend is not to find the `best' evaluation metric:
...
%DIF >  \subsection{Descriptive statistics}
%DIF >  
%DIF >  Useful basic statistics to describe and compare datasets are 
%DIF >  the mean, median and standard deviation of cell counts in the
%DIF >  simulated and known aggregate-level database.
%DIF >  These are useful `sanity checks': if the mean population across all zones or
%DIF >  for a particular constraint differs greatly from expectations, there is clearly
%DIF >  a problem. These summary statistics are dependent on the
%DIF >  size of the dataset. To compare datasets of widely varying populations,
%DIF >  dimensionless statistics can be used.
%DIF >  The coefficient of variation, the ratio of the standard deviation to the mean is
%DIF >  an example of a dimensionless statistic robust to population size.
 \DIFaddbegin 


\DIFaddend \subsection{Scatter plots\DIFdelbegin \DIFdel{and the coefficient of determination}\DIFdelend }
...
\DIFaddbegin \subsection{\DIFadd{The coefficient of determination}}
\DIFaddend The coefficient of determination (henceforth $R^2$) is the 
...
\DIFdelbegin \DIFdel{The SAE addresses this issue }\DIFdelend \DIFaddbegin \DIFadd{Total absolute error (TAE) is the sum of the absolute (positive)
discrepancies between the simulated
and known population in an area. The standardized absolute error (SAE) is
simply TAE divided by the total population. 
An issue with SAE has been that, }\DIFaddend by using the \DIFdelbegin \DIFdel{population size }\DIFdelend \DIFaddbegin \DIFadd{total population }\DIFaddend as the
denominator \DIFdelbegin \DIFdel{,
...
deemed to }\DIFdelend \DIFaddbegin \DIFadd{for TAE calculations on a subset of cells, the method
can }\DIFaddend understate the size of the errors \DIFdelbegin \DIFdel{. Also, this measure does not provide any information on whether any
...
\mbox{%DIFAUXCMD
\citet{clarke2001regional}
}%DIFAUXCMD
used an error threshold of 80 per cent of the
areas with less than 20 per cent error (SAE \textless 0.20). %DIF >  check!!!
\mbox{%DIFAUXCMD
\citet{smith2007simhealth}
}%DIFAUXCMD
work with a model that simulates with diabetes and
use an error threshold of 10\% (SAE \textless 0.1)
in 90 per cent of the output areas. %DIF >  really??? probably
}\DIFaddend 

\DIFdelbegin \subsection{\DIFdel{Z scores}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdel{Here, the
evaluation mainly takes place at the cell level (for a single constraint in a single area). However, there should also be attention for the overall picture like spatial concentration. }\DIFdelend %DIF >  \subsection{The modified Z-score}
...
\DIFaddend % Spatial indicators etc??? Add equations to above methods soooon

\DIFaddbegin \subsection{\DIFadd{Root mean square error}}
\DIFadd{$RMSE$ is our preferred evaluation metric in this study because...
}
...
%DIF >  \section{Appendix: Formulae for evaluation techniques and additional results}
%DIF >  
%DIF >  The symbols used for values in these equations follows, to the
%DIF >  extent possible, the most common usage found in the literature.
%DIF >  Thus $T_{ij}$ and $U_{ij}$ represent the simulated and observed
%DIF >  cell counts for category $i$ in zone $j$. Additional symbols are
%DIF >  described as and when they appear in the formulae.
%DIF >  
%DIF >  
%DIF >  \subsection{Modified Z-Scores}
%DIF >  
%DIF >  The modified Z-score ($Zm$) was described by \citet{Williamson1998}
%DIF >  and is defined as follows:
%DIF >   
%DIF >  \begin{equation}
%DIF >   Zm_{ij} = (r_{ij} - p_{ij}) \Bigg/ \left(\frac{p_{ij}(1 -
%DIF >  p_{ij}))}{\sum\limits_{ij}U_{ij}}\right)^{1/2}
