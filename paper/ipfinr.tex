...
implementation in code. %Have you implemented in R???
...
\begin{table}[h]
\caption{Summary results of standard absolute error (SAE) between
...
\midrule
\textbf{Baseline} &  & 0.018  & \\
\midrule
\textbf{Constraints and iterations} &  &  & \\
1 iteration &  & 1.91 & \\
5 iterations &  & 0.00001  & \\
10 iterations &  & 0.000002  & \\
Reordered constraints 		&  & 0.018 & \\ % it is worse, but only a little!
\textbf{Initial weights} 	&  &  & \\
10\% sample set to 10  		&  & 0.034  & \\
10\% sample set to 1000 	&  & 0.998 & \\
\midrule
\textbf{Empty cells} 		&  &  & \\
1 additional empty cell 	&  & 0.018  & \\
10\% additional empty cells 	&  & 0.050  & \\
No empty cells			&  & 0.005 & \\
\midrule
...
A direct recommendation from these findings is that researcher using spatial microsimulation
