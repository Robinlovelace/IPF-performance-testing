...
to a `wide' format (where each row represents a geographic area or other sub-group of 
the total population and each column represents a variable value).
This process of aggregation is performed in three steps:  a) conversion of all 
...
Iterative proportional fitting (IPF) can help overcome these issues when used as
a basis for \emph{spatial microsimulation}, which can be understood in simplistic terms
as the reversal of geographic aggregation to approximate the original `long' dataset (ref). 
As illustrated in \cref{fmsim-schema}, the need for spatial microsimulation is driven by 
the tendency of statistical agencies to make the raw georeferenced microdata generated 
through large social surveys unnavailable to the research community: spatial microsimulation
is also known as \emph{population synthesis}, especially in transport modelling, which 
conveys its core function: to generate --- and then, in most cases, analyse ---
long spatial microdata that approximates the raw spatial microdata collected during surveys.
...
ability to \emph{disaggregate} the geographically aggregated data described above, 
resulting in the generation of synthetic \emph{spatial microdata}
--- a long table of individual-level data whereby each individual is also allocated to 
a geographic zone.
...
in the wider context of the availability of different data formats and spatial microsimulation.}
\end{figure}
